...
4. 启动容器  
  ```docker run -idt --name ubnt-mips-shadowsocks-libev --network=host ubnt-mips-shadowsocks-libev```
5. 从容器中拷贝  
...
由 ENV 参数 ARCHITECH 判断，默认生成 mips，需要 mips64 在第 4 步的时候替换成下面的命令  
  ...
  ```docker run -idt --name ubnt-mips-shadowsocks-libev --network=host -e ARCHITECH="mips64" ubnt-mips-shadowsocks-libev```
